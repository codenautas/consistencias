...
export class ConCompiler extends ExpressionProcessor{
    ...
    protected buildClausulaFrom(con:Consistencia): string {
        return 'FROM ' + this.buildEndToEndJoins(con.tdsInvolvedInExpr) + this.buildOptRelationsFromClausule(con.insumosOptionalRelations);
    }
    ...
    prepareEC(con:Consistencia): any {
        con.prepare();
        super.prepareEC(con);
        con.expresionProcesada = this.getWrappedExpression(con.expresionProcesada, con);
